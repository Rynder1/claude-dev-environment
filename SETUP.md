...
```bash
scripts/add-repo.sh https://github.com/your-org/your-repo
# or:  scripts/add-repo.sh your-org/your-repo
# or:  scripts/add-repo.sh your-org/your-repo myrepo     # custom name
```
...
```bash
git clone https://github.com/your-org/your-repo ~/code/your-repo
```
...
```bash
scripts/new-env.sh --repo ~/code/your-repo
```
Note the **port** and **folder** it prints (e.g. port `2201`, folder `/workspaces/SSH-your-repo`).
...
```bash
scripts/setup-git-auth.sh SSH-your-repo
```
...
```
Type **`yes`** when asked. You should land on a prompt like `node@ssh-your-repo` with **no
password**. Type `exit`.
...
| Folder | `/workspaces/SSH-<RepoName>` (e.g. `/workspaces/SSH-your-repo`) |
...
- **Rebuild a broken container, keep history:** `scripts/rebuild.sh SSH-your-repo`.
...
```bash
echo "<your-token>" | scripts/setup-git-auth.sh SSH-your-repo --stdin
```
...
```bash
scripts/new-env.sh --repo ~/code/your-repo --win-key /mnt/c/Users/<you>/.ssh/id_ed25519.pub
```
